...
contract Ownable {
    ...
    ) public 
    {
        ...
        emit OwnerShipIsTransferedEvent(_owner);
    }

    //  3) create an 'onlyOwner' modifier that throws if called by any account other than the owner.
    modifier onlyOwner() {
        require(msg.sender == _owner, "Not enable cause It's not the owner");
        _;
    }
    ...
    function transferOwnership(
        ...
        emit OwnerShipIsTransferedEvent(newOwner);
    }
    
    // 5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event OwnerShipIsTransferedEvent(address newOwner);
}
...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused() {
        require(!_paused, "Paused is true when expected false");
        _;
    }
    modifier paused() {
        require(_paused, "Paused is false when expected false");
        _;
    }

    //  5) create a Paused & Unpaused event that emits the address that triggered the event
    event PausedEvent(address addressTriggerer);
    event UnpausedEvent(address addressTriggerer);
}
...
contract ERC721 is Pausable, ERC165, usingOraclize {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(_tokenOwner[tokenId] != to,"Already owner of tokenId");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == _tokenOwner[tokenId],"MsgSender isn't the owner of the contract");
    ...
    returns (bool) {
        ...
        bool spenderIsOwner = spender == owner;
        /*require(1==0,
        strConcat(
            addressToString(spender),
            addressToString(owner),
            uint2str(tokenId),
            uint2str(spenderIsOwner ? 1 : 0)));*/
        bool tokenIdIsApprovedBySpender = getApproved(tokenId) == spender;
        bool isApprovedByAll = isApprovedForAll(owner, spender);
        
        return (
            spenderIsOwner 
            || 
            tokenIdIsApprovedBySpender
            || 
            isApprovedByAll
            );
    }
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        address emptyAddress = address(0);
        require(to != emptyAddress 
            && !_exists(tokenId), "Token Already exists or address is invalid");

        // TODO mint tokenId to given address & increase token count of owner
        address oldOwner = _tokenOwner[tokenId];
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId] == from, 
            strConcat("Token owner isn't the owner",addressToString(from),addressToString(to)));
        
        address emptyAddress = address(0);
        // TODO: require token is being transfered to valid address
        require (to != emptyAddress, 
            strConcat("To isn't valid address",addressToString(to)));
        // TODO: clear approval
...
contract ERC721Metadata is ERC721Enumerable {
    ...
    function setSymbol(string calldata symbol) 
    external {
        _symbol = symbol;
    }
